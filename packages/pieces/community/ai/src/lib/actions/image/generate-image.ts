...
import { aiAuth } from '../../common/auth';
...
export const generateImageAction = createAction({
  ...
  props: {
    ...
    advancedOptions: Property.DynamicProperties({
      ...
      props: async (propsValue): Promise<InputPropertyMap> => {
        ...
        const rawProvider = (auth?.['provider'] || auth?.['props']?.['provider']) as AIProviderName | undefined;
  ...
  async run(context) {
    ...
    const provider = auth?.props?.provider as AIProviderName;
    const apiKey = auth?.props?.apiKey as string;
    const modelId = context.propsValue.model as string;
